...
constructor(type,x,y) {
	...
	let id = Object.keys(math_nodes).length;
	this.check_id(id);
	this.sockets = {};
}

check_id (new_id) {
	let long_id = "node_"+new_id;
	if (math_nodes[long_id] === undefined) {
		this.id = new_id;
		this.name_id = long_id;
	}else {
		new_id++;
		this.check_id (new_id)
	}
}
	...
	const s_info = {
		value:{
			...
			protocol:function(elO,elI) {
				...
				if (elO.tagName == "INPUT") {
					protocol.value = elO.value;
				}else {
					protocol.value = elO.innerHTML;
				}
				return protocol;
			}
		},
		types:{
			...
			protocol:function(elO,elI) {
				//* el1 is output
				//* el2 is input
				elI.setAttribute("type",elO.value);
				return false;
			}
		}
	}
...
let nodes = {
	"input":{
		//*This is a function where I'm creating new nodes
		"create":function() {
				return crtEl("input",{"type":"text","class":"node_inp"});
			},
		...
		//* THIS IS INSTRUCTION FOR COMMUNICATION AND HERE I WROTE WHAT TO DO WITH INFORMATION
		//* c_instruction is also can be c_ins
		c_instruction: function (oth_info,sktO,sktI) {
			if (oth_info.flow=="input") {
				if (oth_info.elO.type == oth_info.elI.type) {
					this.el.node_cont.value = oth_info.value;
				}
			}
		}
	},
	...
	"math": {
		"create":function() {
				return crtEl("div",{"class":"math_inp"});
		},
		'sockets':{input:[s_info.value],output:[]},
		c_instruction: function (oth_info,sktO,sktI) {
			if (oth_info.flow=="input") {
				let val = oth_info.value;
				let calculation = val;
				let last_signs = val.slice(-1);
				try{
					//* sourse https://mathjs.org/
					calculation = math.evaluate(val);
					//*endsourse
				}catch (e) {
					calculation = val;
				}
				...
				this.el.node_cont.innerHTML = calculation;
			}
		}
	}
...
add_content(type) {
	//* Give node instruction how to communicate
	this.c_ins = this.BD_of_nodes().c_instruction;
	...
	for (let v of Object.values(my_node.sockets)) {
		if (v.connections !== undefined && v.connections != ""){
			...
			if (v.flow == "output") {
				...
				const my_socket = v;
				if (my_socket.type == oth.socket.type) {
					oth.el = oth.node.el.node_cont;

					var ins_var = s_info[my_socket.type].protocol(my_el, oth.el);
					if (ins_var != false && ins_var.flow == "input" || ins_var.flow === undefined) {
						oth.node.c_ins(ins_var,my_socket,oth.socket);
					}
					if (ins_var != false && ins_var.flow == "output") {
...
remove_Math_Node(remove_it) {
	if (remove_it) {
		setTimeout(
			()=>{
			for (let v of Object.values(this.sockets)) {
				if (v.connections !== undefined && v.connections != "") {
					let oth_line = v.connections.socket.$el.querySelector(".line");
					oth_line.style.width = "0px";
					oth_line.style.transform = "rotate(0deg)";
					console.log(oth_line)
					v.connections.socket.connections = ""
				}
			}
			this.$el.remove();
			delete math_nodes[this.name_id];
		},100)
	}
}
...
function keepConnection(e,r_arg,status,x,y) {
	//* REMOVE PART
	if (status == 0) {
		if (Math_node_delete_but !== undefined) {
			let rmv = false;
			let dlt_b_Crd = find_objCrd(Math_node_delete_but);
			// console.log(dlt_b_Crd)
			// console.log(x+" "+y)
			if (dlt_b_Crd.x <= x && dlt_b_Crd.x_bd() >= x 
				&& dlt_b_Crd.y <= y && dlt_b_Crd.y_bd() >= y) {
				rmv = true;
				r_arg.remove_Math_Node(rmv);
			}
		}
	}
	//* KEEP CONNACTION PART
	const node = r_arg;
...
let Math_node_delete_but = "";
//*ADDING NEW NODE 
function add_math_node(box_place,node_type,node_pattern,x,y,dlt_but){
	Math_node_delete_but = dlt_but;
	...
	node = new Math_node(node_type,x,y);
...
window.onload = ()=> {
// add_math_node(getEl('.sand_box'),"input",getEl(".node_item",1)[0],200,60);

// add_math_node(getEl('.sand_box'),"input",getEl(".node_item",1)[0],400,100);
