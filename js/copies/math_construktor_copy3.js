...
create_socket(which,flow,type) {
	...
	socket.setCSS({"background":sck_info.background,"border":sck_info.border,"z-index":10});
...
function Fdfex_conect_sckt (e,status,obj,x,y,){
	...
	if (status == 1) {
		var skt_z = obj.socket.zIndex+1;
		obj.socket.$el.style.zIndex = skt_z;

		for (const [key,value] of Object.entries(math_nodes)) {
			if (key != obj.node.name_id) {
				Object.keys(math_nodes[key].sockets).forEach(function(el,i) {
					console.log(el)
					console.log(math_nodes[key].sockets[el].crds.x)
				});
			}
		}
	}
	if (status == 0) {
		var skt_z = obj.socket.zIndex;
		obj.socket.$el.style.zIndex = skt_z;
		return false;
	}
